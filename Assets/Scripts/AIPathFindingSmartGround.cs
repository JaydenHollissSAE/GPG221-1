...
public class AIPathFindingSmartGround : AIPathFindingGround
{
    ...
    protected virtual void CheckForItems()
    {
        ...
        {
            if (neededItems.Contains(item.type))
            {
                ItemNeeded(item.type);
                foundMatch = true;
            }
        }
    ...
    protected virtual void ItemBehaviour()
    {
        ...
        else
        {
            ...
            {
                if (!NewUnity.ContainsV3(lockedPositions, itemInFront.position)) lockedPositions.Add(itemInFront.position);
